    ...
    <main>
      ...
      <section>
        <div class="suggest-content">
          ...
          <div class="option">
            <a href="<?php echo _DEFAULT_PATH ?>/tvshows"><h5>Popular</h5></a>
            <a href="<?php echo _DEFAULT_PATH ?>/tvshows/top"><h5>Top Rated</h5></a>
            <a href="<?php echo _DEFAULT_PATH ?>/tvshows/air"><h5>On The Air</h5></a>
          </div>
        </div>
        <div class="wrapper-all-show">
        <?php
        foreach($tvs as $obj){
          $year = $obj->get('first_air_date');
          $year = strtok($year, '-');
          echo '
          <div class="show-component">
            <div class="movie-card">
              <a href="'._DEFAULT_PATH.'/detail/tv/'.$obj->getID().'">
                <figure class="card-banner">';
                if(!empty($obj->get('poster_path'))){
                  echo '<img src="'._TMDB_IMG.$obj->get('poster_path').'" />';
                }else{
              ...
              <div class="title-wrapper">
                <a href="'._DEFAULT_PATH.'/detail/m/'.$obj->getID().'">
              ...
              <div class="card-meta">
                <div class="badge badge-outline">'.strtoupper($obj->get('original_language')).'</div>

                <div class="duration">
                  <ion-icon name="people"></ion-icon>

                  <time>'.$obj->get('vote_count').'</time>
                </div>

                <div class="rating">
                  <ion-icon name="star"></ion-icon>

                  <data>'.round($obj->get('vote_average'), 1).'</data>
                </div>
              </div>
            </div>
          </div>
      ...
      <div class="next-page">
        <div class="next-page-control">
          <?php
          if($page > 1 && $page <= 10){
            echo '<a href="'._DEFAULT_PATH.'/tvshows/'.$type.'/'.($page - 1).'"><i class="fa-solid fa-backward-step"></i></a>';
          }
          ...
          for($i = 1; $i <= 10; $i++){
            if($i == $page){
              ...
              echo '<a href="'._DEFAULT_PATH.'/tvshows/'.$type.'/'.$i.'"><div class="number-page">'.$i.'</div></a>';
            }
          }

          if($page < 10 && $page >= 1){
            echo '<a href="'._DEFAULT_PATH.'/tvshows/'.$type.'/'.($page + 1).'"><i class="fa-solid fa-forward-step"></i></a>';
          }
          ?>
        </div>
      </div>
