      ...
      no-repeat; background-size: cover;">
        <div class="container">
          ...
          <div class="movie-detail-content">
            <?php
            if(!empty($detail->get('belongs_to_collection'))){
              echo '<p class="detail-subtitle">From "'.$detail->get('belongs_to_collection')['name'].'"</p>';
            }
            ?>

            <h1 class="h1 detail-title"><strong><?php echo $detail->getTitle(); ?></strong></h1>

            <div class="meta-wrapper">
              <div class="badge-wrapper">
                <div class="badge badge-outline"><?php echo strtoupper($detail->get('original_language')); ?></div>
              </div>
              ...
              <div class="date-time">
                <div>
                  ...
                  <time><?php echo $detail->get('release_date'); ?></time>
                </div>
                <?php
                if(!empty($detail->get('runtime'))){
                  echo '
                  <div>
                    <ion-icon name="time-outline"></ion-icon>
                    <time>'.$detail->get('runtime').' min</time>
                  </div>
                  ';
                }
                ?>
            ...
            <div class="details-actions">
              ...
              if(!empty($_SESSION['user_username'])){
                ...
                echo '
                <button class="btn btn-primary" onclick="needLoginFirst()">
                  <i class="fa-solid fa-circle-plus"></i>
                  <span>Favorite</span>
                </button>
                ';
...
if(empty($detail->get('trailers')['youtube']) && empty($detail->get('images')['posters']) && empty($detail->get('images')['backdrops'])){
  ...
  echo '
          <div class="wrapper-media">
            ...
            <div class="carousel">';
              $trailers = $detail->get('trailers')['youtube'];
              if(!empty($trailers)){
                if(count($trailers) == 1){
                  echo '<iframe width="560" height="360" src="https://www.youtube.com/embed/'.$trailers[0]['source'].'?si=ELU48noR33BJMqwV" title="YouTube video player" frameborder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture; web-share" allowfullscreen></iframe>';
                }else{
                  for($i = 0; $i < 2; $i++){
                    echo '<iframe width="560" height="360" src="https://www.youtube.com/embed/'.$trailers[$i]['source'].'?si=ELU48noR33BJMqwV" title="YouTube video player" frameborder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture; web-share" allowfullscreen style="margin-left: 15px;"></iframe>';
                  }
                }
              }
                  ...
                  <div class="body-report">
                    <div class="inf-film-report">
                      ...
                      <div class="inf-film-report-text">
                        <p>'.$detail->getTitle().' ('.strtok($detail->get('release_date'), '-').')</p>
